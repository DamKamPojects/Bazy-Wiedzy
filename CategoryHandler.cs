...
namespace BW_PrototypWPF
{
    public class CategoryHandler : IFileHandler
    {
        ...
        {
            var ToWrite = previousText + Environment.NewLine + insert.ID + "," + insert.Name + RestOfTheFile;
        ...
        private List<string> StringForUpdateAndDelete(string filename, int id)
        {
            ...
            using (StreamReader srChecking = new StreamReader(Path, Encoding.Default))
            {
                ...
                while (!srChecking.EndOfStream)
                {
                    ...
                    if (line[0] != id.ToString() && i != 0 && SavePreviousLines) previousString += Environment.NewLine;
